...
Traditional chatbots are stateless: they don't remember past conversations, can't learn from successful resolutions, and treat every customer as a stranger. This leads to:
...
This agent follows a six-step cognitive loop:

1. **Perceive & Classify**: Extract intent, detect sentiment, identify customer
2. **Recall (Episodic Memory)**: Search past tickets and resolution patterns
3. **Orient (Working Memory)**: Get current conversation state and context
4. **Decide (LLM)**: Generate response with full context
5. **Act**: Execute skills, send response, trigger escalation if needed
6. **Learn & Update**: Commit interaction to memory, record outcome
...
│   ├── memory.ts             # Rice Slate integration
  ...
  async handleMessage(
    ...
    if (llmResult.suggestedAction === "refund" && classification.entities.orderId) {
      ...
      if (refundResult.eligible) {
        ...
        finalResponse += `\n\n✅ I've processed a refund of $${refundResult.amount.toFixed(2)} to your original payment method. ${refundResult.policy}`;
      } else {
    ...
    if (llmResult.suggestedAction === "escalate" || llmResult.confidence < 0.4) {
      ...
      if (escalation.shouldEscalate || llmResult.confidence < 0.4) {
        ...
        finalResponse += `\n\n🔄 I'm connecting you with a specialist who can better assist with this. They'll be with you shortly.`;
      }
    }
...
- ✅ Maintains conversation context with **Working Memory**
- ✅ Learns from past resolutions with **Episodic Memory**
- ✅ Executes deterministic actions with **Procedural Memory**
- ✅ Improves over time through continuous learning
- ✅ Handles escalations intelligently

The agent demonstrates the core cognitive loop: Perceive → Recall → Orient → Decide → Act → Learn.

::callout{icon="i-lucide-arrow-right" color="primary"}
**Next Tutorial**: [Research Assistant Agent](/tutorials/research-assistant). Build an agent that synthesizes knowledge across sessions and builds a personalized understanding of user interests.
::
