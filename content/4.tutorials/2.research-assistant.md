...
1. **Understand Query**: Extract research topic, identify sub-questions, connect to past research
2. **Recall Context (Episodic)**: Find related past sessions, retrieve preferences and useful sources
3. **Load Session State (Working)**: Get current session notes and active research threads
4. **Research & Synthesize**: Search external sources, cross-reference, generate synthesis
5. **Format & Deliver (Procedural)**: Format citations, validate credibility, structure response
6. **Learn & Update**: Record session, update interest profile, store sources for future use
...
│   ├── memory.py             # Rice Slate integration
...
async def main():
    ...
    for interest in profile['top_interests'][:5]:
        print(f"  • {interest['topic']} (strength: {interest['strength']:.2f})")
...
Memory State:
  ...
  - Interest: user → transformers (strength: 0.1)
```
...
Memory State:
  ...
  - Knowledge Graph: transformers → attention → long sequences
  - Interest: user → transformers (strength: 0.4, depth increasing)
```
...
Memory State:
  ...
  - Interest: user → transformers (strength: 0.9, expert level)
```
...
- ✅ Maintains session context with **Working Memory**
- ✅ Learns from past research with **Episodic Memory**
- ✅ Validates sources and formats citations with **Procedural Memory**
- ✅ Builds a personal knowledge graph over time
- ✅ Tracks user interests for personalization
- ✅ Improves synthesis quality with accumulated context
