...
description: Understanding Flux—the attention-based working memory system.
navigation:
  icon: i-lucide-layers
seo:
  title: Flux - Working Memory
  description: Learn how Flux provides dynamic, attention-based working memory for AI agents.
---

**"The Attention Span"**

Flux manages the agent's immediate context. Unlike a raw context window, Flux is dynamic—items decay over time if not accessed, simulating natural forgetting.
...
Items in Flux naturally decay over time if they aren't accessed. This prevents context overload and ensures the agent focuses on what's currently relevant.
...
When you read from Flux using `drift()`, items are returned sorted by their **Attention Score**—a combination of recency and access frequency.
...
**Don't fight decay**: If something is important long-term, use Echoes (episodic memory) instead.
::
