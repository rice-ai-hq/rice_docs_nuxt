...
description: Understanding Reflex—the procedural memory system for executing compiled skills.
navigation:
  icon: i-lucide-zap
seo:
  title: Reflex - Procedural Memory
  description: Learn how Reflex enables AI agents to execute deterministic, sandboxed skills via WebAssembly.
---

**"The Muscle Memory"**

Reflex executes compiled skills (WebAssembly) server-side. These are deterministic, sandboxed procedures that agents can trigger—like muscle memory that doesn't require thinking.
...
Never let an LLM calculate taxes or financial figures—use Reflex.
...
**Use for calculations**: Any math that must be correct should go through Reflex, not the LLM.
::

::callout{icon="i-lucide-shield" color="amber"}
**Security-sensitive operations**: Validation, sanitization, and permission checks belong in Reflex.
::
